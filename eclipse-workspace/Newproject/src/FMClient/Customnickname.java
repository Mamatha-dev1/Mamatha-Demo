...
public class Customnickname {
	...
	public void executeNickNamecreation(WebDriver driver) throws InterruptedException, IOException {
		try {
			...
			WebElement StandardnicknameLink = wait
					.until(ExpectedConditions.elementToBeClickable(By.id("id-Nicknames Custom")));
		...
		try {
			...
			WebElement descriptionInput = wait
					.until(ExpectedConditions.visibilityOfElementLocated(By.id("flicDescription")));
		...
		try {
			...
			WebElement nicknameField = wait
					.until(ExpectedConditions.visibilityOfElementLocated(By.id("nickname-direct-value")));
			nicknameField.clear();
		...
		try {
			WebElement Savebutton = wait
					.until(ExpectedConditions.elementToBeClickable(By.id("customNicknameDetail.save")));
			Savebutton.click();
		...
		try {
			WebElement Adavncefilterbutton = wait
					.until(ExpectedConditions.presenceOfElementLocated(By.id("advanced-filter-button")));
			...
			WebElement rulenameElement = driver
					.findElement(By.xpath("//input[@type='text' and @class='gwt-SuggestBox original-token-input']"));
		...
		try {
			WebElement Nicknamebutton = wait
					.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[@style='outline-style:none;']")));
		...
		try {
			WebElement Nicknameactionsbutton = wait.until(
					ExpectedConditions.elementToBeClickable(By.xpath("//div[contains(text(),'Nickname Actions')]")));
		...
		try {
			WebElement Editbutton = wait.until(
					ExpectedConditions.elementToBeClickable(By.xpath("//div[contains(text(),'Edit Contents')]")));
		...
		Thread.sleep(2000);
		try {
			WebElement deletebox = driver.findElement(By.xpath(
					"//td[contains(@style,'vertical-align: top;')]//div[@class='gwt-label-stack-delete-icon-holder']"));

			Actions a = new Actions(driver);

			while (true) {
				try {
					// Perform the action of hovering and clicking the delete icon for the current
					// deletebox
					a.moveToElement(deletebox).build().perform();

					By deleteIconLocator = By.cssSelector(".gwt-label-stack-delete-icon");

					WebElement deleteIcon = wait
							.until(ExpectedConditions.visibilityOfElementLocated(deleteIconLocator));
					...
					deletebox = driver.findElement(By.xpath(
							"//td[contains(@style,'vertical-align: top;')]//div[@class='gwt-label-stack-delete-icon-holder']"));
		...
		try {
			...
			// Ensure the "Add Value" button is clickable and that the field is ready for
			// input
			WebElement addButton = wait.until(ExpectedConditions.elementToBeClickable(By.id("nickname-add-value")));
		...
		try {
			WebElement minimizebutton = wait
					.until(ExpectedConditions.elementToBeClickable(By.xpath("//a[@class='roc-dialog-close']")));
		...
		try {
			...
			WebElement Deletebutton = wait
					.until(ExpectedConditions.elementToBeClickable(By.xpath("//div[contains(text(),'Delete')]")));
