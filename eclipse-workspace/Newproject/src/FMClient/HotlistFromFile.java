...
public class HotlistFromFile {
	...
	public void HotlistPage() throws InterruptedException, IOException, TimeoutException {
		...
		// Use the driver from LoginManager (static method should be accessed via class
		// name)
	...
	public void executeHotlistcreation(WebDriver driver) throws InterruptedException, IOException, TimeoutException {
		...
		try {
			...
			WebElement HotlistgroupLink = wait
					.until(ExpectedConditions.elementToBeClickable(By.id("id-Hotlist Groups")));
		...
		try {
			...
			// Click the User link after typing
			WebElement HotlistgroupLink = wait
					.until(ExpectedConditions.elementToBeClickable(By.id("id-Hotlist Entities")));
			HotlistgroupLink.click();
//           wait.until(ExpectedConditions.visibilityOfElementLocated(By.id("yes"))).click();
		...
		try {
			// Wait for the "Common Tasks" element to be visible
			WebElement commonTasks1 = wait
					.until(ExpectedConditions.visibilityOfElementLocated(By.xpath("//div[@id='Common Tasks']")));
			Actions action = new Actions(driver);
		...
		try {
			// Wait for 5 seconds before interacting with the upload button (ensure the
			// button is clickable)
			Thread.sleep(5000);

			WebElement uploadButton = driver
					.findElement(By.xpath("//form[@id='fileUpload']/table/tbody/tr/td/table/tbody/tr/td/div"));
			uploadButton.click();
		...
		try {
			// Enter username
			WebElement selectAllbutton = wait
					.until(ExpectedConditions.elementToBeClickable(By.id("expand-hotlistGroups")));
			...
			// Change the CSS selector based on the actual structure of your page
		...
		try {
			...
			WebElement Donebutton = wait
					.until(ExpectedConditions.elementToBeClickable(By.xpath("//button[normalize-space()='Done']")));
