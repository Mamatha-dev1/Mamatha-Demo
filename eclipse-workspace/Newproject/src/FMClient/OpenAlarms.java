...
public class OpenAlarms {
    ...
    @Test
    public void testOpenAlarms() throws InterruptedException {
                        
        // Use the driver from LoginManager
        executeOpenAlarms(loginManager.getDriver());
        System.out.println("Open Alarms test executed");
    }
  
    public static void executeOpenAlarms(WebDriver driver) throws InterruptedException {
        ...
        try {
            ...
            WebElement navigationLabel = wait.until(ExpectedConditions.elementToBeClickable(By.id("navigationLabel")));
            navigationLabel.click();
            System.out.println("Clicked navigation label successfully.");

            // Find the search input field, enter "Alarms Open"
            WebElement searchInput = wait.until(ExpectedConditions.elementToBeClickable(By.xpath("//input[@placeholder='Type to search']")));
            searchInput.sendKeys("Alarms Open");
            System.out.println("Entered 'Alarms Open' in search input successfully.");

            // Locate and click the "id-Alarms Open" element
            WebElement alarmsOpenElement = wait.until(ExpectedConditions.elementToBeClickable(By.id("id-Alarms Open")));
            alarmsOpenElement.click();
            ...
            try {
                ...
                WebElement openAlarmsLabel = extendedWait.until(ExpectedConditions.presenceOfElementLocated(
                    By.cssSelector("div.gwt-HTML.layout-title-bar-text#filterpanel-header-label")));
                
                // Check if the text is "Open Alarms Search"
                String labelText = openAlarmsLabel.getText();
                if ("Open Alarms Search".equals(labelText)) {
                    System.out.println("Navigated to 'Open Alarms' successfully.");
                } else {
                    System.out.println("Unexpected label found: " + labelText);
                }
            } catch (TimeoutException e) {
                System.out.println("'Open Alarms Search' label was not found, navigation might have failed.");
            }
            Thread.sleep(3000);
